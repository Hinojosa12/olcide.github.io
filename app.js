...
function doLogout() {
  currentUser = null; myStatus = 'idle'; activityLog = []; userAllowedBrands = [];
...
async function fetchMessages() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (userAllowedBrands.length > 0) {
        allMessages = allMessages.filter(m => userAllowedBrands.includes(m.brand));
      }
      buildConversations();
      buildBrandFilters();
      renderInbox();
...
function buildConversations() {
  ...
  filtered.forEach(m => {
    ...
    if (!conversations[key]) {
      conversations[key] = { id:key, senderId:m.senderId, senderName:m.senderName, brand:m.brand, messages:[], lastMessage:null, lastTime:null, unread:0 };
    }
...
function renderInbox() {
  ...
  if (!convos.length) {
    list.innerHTML = '<div class="msg-empty" style="padding:2rem"><i class="fas fa-comments" style="font-size:1.2rem;opacity:.4;display:block;margin-bottom:.5rem"></i>No conversations</div>';
    return;
  }
  ...
  list.innerHTML = convos.map(c => {
    ...
    return `<div class="inbox-item${isActive}" onclick="openConversation('${c.id.replace(/'/g, "\\'")}')">
      ...
      <div class="inbox-meta">
        ...
        <span class="inbox-brand-tag">${escapeHtml(c.brand)}</span>
        <span class="inbox-msg-count">${c.messages.length}</span>
      </div>
    </div>`;
  }).join('');
}
...
function openConversation(convoId) {
  ...
  const delBtn = document.getElementById('btnDeleteConvo');
  delBtn.style.display = (currentUser && currentUser.role === 'admin') ? 'flex' : 'none';
  ...
  msgs.forEach(m => {
    ...
    if (m.attachmentUrl) {
      ...
      if (aType === 'audio') {
        attachHtml = `<audio controls style="max-width:240px;margin-bottom:.3rem" preload="none"><source src="${escapeHtml(m.attachmentUrl)}">Audio not supported</audio><div class="msg-img-expired" style="display:none"><i class="fas fa-microphone" style="margin-right:4px"></i>Audio expired</div>`;
      } else if (aType === 'video') {
        attachHtml = `<video controls style="max-width:260px;max-height:200px;border-radius:8px;margin-bottom:.3rem" preload="none"><source src="${escapeHtml(m.attachmentUrl)}">Video not supported</video><div class="msg-img-expired" style="display:none"><i class="fas fa-video" style="margin-right:4px"></i>Video expired</div>`;
      } else if (aType === 'file') {
        attachHtml = `<a href="${escapeHtml(m.attachmentUrl)}" target="_blank" style="display:inline-flex;align-items:center;gap:.4rem;padding:.4rem .7rem;background:var(--accent-glow);border:1px solid rgba(59,130,246,.3);border-radius:8px;color:var(--accent);font-size:.78rem;font-weight:600;text-decoration:none;margin-bottom:.3rem"><i class="fas fa-file-download"></i> Download File</a>`;
      } else {
        attachHtml = `<img class="msg-bubble-img" src="${escapeHtml(m.attachmentUrl)}" alt="Photo" onclick="window.open(this.src,'_blank')" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"><div class="msg-img-expired" style="display:none"><i class="fas fa-image" style="margin-right:4px"></i>Image expired</div>`;
      }
    }

    const msgText  = m.messageText || '';
    const textHtml = (msgText && !(m.attachmentUrl && placeholders.includes(msgText)))
    ...
    html += `<div class="msg-row ${dir}">
      <div class="msg-bubble">
        ...
        <div class="msg-bubble-footer">
          <span class="msg-bubble-time">${dir === 'sent' ? '<i class="fas fa-reply" style="font-size:.45rem;margin-right:2px"></i> ' : ''}${timeStr}</span>
        </div>
      </div>
    </div>`;
  });
...
async function deleteConversation() {
  ...
  try {
    ...
    const data = Array.isArray(raw) ? raw[0] : raw;
    if (data.success) {
      delete conversations[activeConvoId];
      activeConvoId = null;
...
async function sendReply() {
  ...
  try {
    ...
    if (data.success) {
      ...
      const newMsg = { timestamp:now, brand:convo.brand, pageId, senderId:convo.senderId, messageText:text, messageId:data.messageId||'sent_'+Date.now(), platform:'Messenger', direction:'sent', senderName:convo.senderName };
